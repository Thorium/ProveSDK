...
module Identity =
    ...
    type ProveVerifyRequest = FSharp.Data.JsonProvider< """[{
    ...
    type internal ProveVerifyResponse = FSharp.Data.JsonProvider<"prove-verify-response.json", SampleIsList=true>
    ...
    let callProveVerify license (phonenr:string) firstname lastname city zipCode stateCode (dateOfBirth:DateTime option) (ssn:string option) =
        async {
            ...
            let req = ProveVerifyRequest.Root(Guid.NewGuid().ToString(), phone, firstname, lastname, None, city, Some true,
                        ...
                        ssn |> Option.map(fun ssnNr -> ssnNr.Replace(" ", "").Replace("-", ""))).ToString()
            ...
            match res with
            | r, None ->
                let parsedResp = ProveVerifyResponse.Parse r
...
module Prefill =
    ...
    type ProveIdentityRequest = FSharp.Data.JsonProvider< """[{
    ...
    },{"requestId":"1230544e-0f2d-1230-8149-123658bad63d","status":1000,"description":"Parameter is invalid.","additionalInfo":"dob invalid."}]""", SampleIsList=true>

    /// Calls Pre-Fill
    let provePrefillIdentity license (phone:string) (dateOfBirth:DateTime) (ssn:string option) =
        async {

            let! auth = ServiceCall.proveAuth license

            let phonenr = phone.Replace(" ", "").Replace("-", "").Replace("(","").Replace(")","").Replace("+","") |> Int64.Parse
            let ssnnr = ssn |> Option.map(fun s -> s.Replace(" ", "").Replace("-", "").Replace("(","").Replace(")","").Replace("+",""))
            let req = ProveIdentityRequest.Root(Guid.NewGuid().ToString(), phonenr, (Some (dateOfBirth.ToString("yyyy-MM-dd"))), ssnnr).ToString()
            ...
            | r, None ->
                let parsedResp =
                    try
                        let resp = ProveIdentityResponse.Parse r
